...
本动手实验将指导你设置一个完整的开发环境，用于构建与PostgreSQL集成的MCP服务器。你将配置所有必要的工具，部署Azure资源，并在实施之前验证你的设置。

## 概述

一个适当的开发环境对于成功开发MCP服务器至关重要。本实验提供了逐步指导，帮助你设置Docker、Azure服务、开发工具，并验证所有组件是否正确协同工作。

完成本实验后，你将拥有一个完全功能化的开发环境，准备好构建Zava Retail MCP服务器。
