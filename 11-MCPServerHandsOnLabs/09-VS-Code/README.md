...
本实验为你提供全面的指导，帮助你将 MCP 服务器与 VS Code 集成，以通过 AI 聊天实现自然语言查询。你将学习如何配置 VS Code 以优化 MCP 的使用、调试服务器连接，并充分利用 AI 辅助的数据库交互功能。

## 概述

VS Code 的 MCP 集成彻底改变了开发者通过自然语言与数据库和 API 交互的方式。通过将你的零售 MCP 服务器连接到 VS Code Chat，你可以使用对话式 AI 智能查询销售数据、产品目录和业务分析。
